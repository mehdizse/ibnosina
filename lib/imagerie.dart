...
class Imagerie extends StatefulWidget {
  Imagerie({Key key, this.title}) : super(key: key);
...
class _ListPageState extends State<Imagerie> {
  var refreshKey = GlobalKey<RefreshIndicatorState>();
  List data;
  Map<String,dynamic> patient;
  List<dynamic> bilans=[];
  List<dynamic> bilanss=[];
  var ye;
  var nom;
  double _imageHeight;
  var photo;
    ...
    setState(() {
      ...
      if(response.statusCode==200) {
        ...
        for(int i=0;i<bilans.length;i++){
          if(bilans[i]["fichier"]!=null) {
            bilanss.add(bilans[i]);
          }
        }
  ...
  Widget _buildBottomPart() {
    return new Padding(
      ...
      child: new Column(
        ...
        children: <Widget>[
          new Expanded(
              child:Images(bilanss,patient,ye),
          ),
        ],
      ),
    );
