...
import 'details/detailPhyto.dart';
import 'dart:async';
import 'dart:convert';
...
class Phytoterapie extends StatefulWidget {
  Phytoterapie({Key key, this.title}) : super(key: key);
...
class _ListPageState extends State<Phytoterapie> {

  List data;
  Map<String,dynamic> patient;
  List<dynamic> phytoterapies;
  var ye;
  var nom;
  double _imageHeight ;
  var photo;
  ...
  Future<String> makeRequest() async {
    ...
    String url = 'http://10.0.2.2:8000/api/phytoterapie/${patient_id}';
    ...
    setState(() {
      if(response.statusCode==200) {
        ...
        phytoterapies= extractdata["Pytotherapie"];
  ...
  Widget build(BuildContext context) {
    ...
    ListTile makeListTile(Map<String, dynamic> phyto,Map<String,dynamic> patien) => ListTile(
        contentPadding:
        ...
        title: Text(
          capitalizeFirstLetter(phyto["produit_naturel_fr"]),
        ...
        onTap: () {
          ...
          Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => DetailPhyto(phytoes:phyto,patien:patient,age: ye,)));
        }
    );

    Card makeCard(Map<String, dynamic> phyto) => Card(
      elevation: 8.0,
      margin: new EdgeInsets.symmetric(horizontal: 10.0, vertical: 6.0),
      child: Container(
        decoration: BoxDecoration(color: Color.fromRGBO(70, 239, 239, 239)),
        child: makeListTile(phyto,patient),
      ),
    );
    ...
    final makeBody = Container(
      ...
      child: ListView.builder(
        ...
        shrinkWrap: true,
        itemCount: phytoterapies == null ? 0 : phytoterapies.length,
        itemBuilder: (BuildContext context, int index) {
          return makeCard(phytoterapies[index]);
        },
      ),
    );
